...
# Este test queda obsoleto en PPI. Por ahora no hay un disconnect en PPI
# def test_connection():
#     mm = MarketManager(timer_off=True)
#     logging.info(f"is_connected : {mm.hb.is_connected()}")
#     mm.connect()
#     logging.info(f"is_connected : {mm.hb.is_connected()}")
#     mm.disconnect()
#     logging.info(f"is_connected : {mm.hb.is_connected()}")

def test_db_conn_cfg():
    print("Testing db connection ...")
    print (creds.db_conn_info)
    try :
        with dblib.dbconnection() as conn:
            assert conn.status == pg_ext.STATUS_READY, f"Connection status is not ready: {conn.status}"
...
def test_time_config():
    with mock.patch("brokerinterface.HybridWrapper") as hb:
...
def mock_books_quotes (ticker, bid, ask, bid_size = 1, ask_size = 1):
    json = '''
    {
    "Bids": [
        {
    ...
    "Date": "2023-01-20T13:50:53.229-03:00",
    "MaxDay": 492.9,
    "MinDay": 455.0,
    "Offers": [
        {
    ...
    "VolumeAmount": 0.0,
    "VolumeCurrency": 0.0,
    "VolumeTotalAmount": 801005829.75
    }
    '''

    return json

def mock_price_quotes (ticker, price):
    json = '''
    {
        "Bids": [
        ],
        "Date": "2023-01-20T13:50:52.561-03:00",
        "MaxDay": 492.9,
        "MinDay": 455.0,
        "Offers": [
        ],
        ...
        "Trade": true,
        "Type": "ACCIONES",
        "VarDay": 0.0617685305591678,
        "VolumeAmount": 300.0,
        "VolumeCurrency": 146970.0,
        "VolumeTotalAmount": 801005829.75
    }'''
    return json


def mock_market_data (ticker, bid, ask, bid_size = 1, ask_size = 1, bid_count = 1, ask_count = 1):
    ...
    data = {"symbol": ticker, "settlement": "A-48HS", "position" : 1, "bid":bid, "ask": ask, "bid_offers_count": bid_count, "ask_offers_count": ask_count, "ask_size":ask_size, "bid_size": bid_size}
...
def test_on_order_book_change():
    ...
    with mock.patch("brokerinterface.PPI") as ppi:
        with mock.patch("brokerinterface.HomeBroker") as hbm:
            ...
            with mock.patch("Config.asset_strategies", {
                ...
                with mock.patch("helpers.get_time") as gt:
                    ...
                    quotes = mock_books_quotes("ARCO", 1920, 1950, 5, 3)
                    with mock.patch("brokerinterface.HybridWrapper.get_last_price") as glp:
                        glp.return_value = 2000
                        mm.hb.on_order_book(quotes)
            ...
            with mock.patch("Config.asset_strategies", {
                ...
                with mock.patch("helpers.get_time") as gt:
                    ...
                    if target_type == "SELL":
                        ...
                        mm.hb.on_order_book(mock_price_quotes("ARCO", 2000))
                        mm.hb.on_order_book(mock_books_quotes("ARCO", 2050, 2060, offer, 20))

                    elif target_type == "SL":
                        mm.hb.on_order_book(mock_price_quotes("ARCO", 1900))
                        mm.hb.on_order_book(mock_books_quotes("ARCO", 1900, 1910, offer, 20))
                    ...
                    if cancel_on_EOM:
                        ...
                        # primero 2 a un precio
                        bid, ask, bid_s, ask_s = 2008, 2020, 2, 5
